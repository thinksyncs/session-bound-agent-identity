...
facts. It is application-protocol neutral at L0 through L2 and does not
depend on Agent2Agent, AGTP, or any other application protocol to supply
...
work includes W3C DID and Verifiable Credentials specifications & A
wallet can store grants, protect confirmation keys, and sign or present
profile material. Wallet metadata, display names, card fields, and
credential labels are not verifier policy and do not replace Identity
Grant, Session Binding Statement, replay, or local-policy checks. \\
...
validation lives in \texttt{pkg/agtp/gatewayroute}. JWT/JWS and
CWT/COSE route-assertion adapters live in \texttt{pkg/agtp}. Runtime
client/server wiring and a full gateway-routed network red-team harness
...
and replay state. \texttt{pkg/agtp} adds JWT/JWS and CWT/COSE
route-assertion adapters that verify protected \texttt{kid}, issuer,
audience, expiry, nonce, grant hash, gateway session-binding hash,
holder-of-key proof hash, replay state, and local route policy before
acceptance. The adapters do not replace runtime client/server wiring or
a full gateway network harness.
...
proof requirements, and JWT/JWS plus CWT/COSE route-assertion adapters.
Local TLS resumption behavior is covered for JWT/JWS acceptance.
Remaining work includes real 0-RTT transport behavior, gRPC-specific
connection pooling, runtime gateway wiring, a full gateway-routed
network harness, randomized property or fuzz generation, and
